...
def step(adjacency_list: Dict[int, set], pool: ThreadPool) -> Dict[int, set]:
    """
    Runs one step:
    ...
    trades = pool.starmap(trade_with_self_loop, kwargs)

    # recompose the new adjacency list
    for trade in trades:
        # for k, v in trade.items():
            # adjacency_list[k] = v
        adjacency_list[trade[0][0]] = trade[0][1]
        adjacency_list[trade[1][0]] = trade[1][1]
...
def run(adjacency_list: dict, nb_steps: int, nb_workers: int) -> dict:
    ...
    """
    with ThreadPool(nb_workers) as pool:
